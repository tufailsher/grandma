...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _sizeAnimation = Tween<double>(
      begin: 1.0,
      end: 0.2, // Shrink to 20% of original size
    ...
    _positionAnimation = Tween<Offset>(
      begin: Offset(0, 0),
      end: Offset(1, -1), // Move to top right
    ).animate(CurvedAnimation(
  ...
  void _updateTextColor(Color newColor) {
    setState(() {
      // _textColor = newColor;
    });
  }

  void _showTextControlModal(BuildContext context) {
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            // Show fixed position mic button after animation completes
  ...
  // Widget to build the animated microphone button
  Widget _buildAnimatedMicButton() {
    // If animation is complete, return an empty container as the real button
    // is now positioned in the top right
    if (_isButtonAnimated) {
      return Container();
    }
    ...
    return AnimatedBuilder(
      animation: Listenable.merge(
          [_sizeAnimationController, _positionAnimationController]),
